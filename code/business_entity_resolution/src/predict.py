...
Scores every candidate pair, applies the decision layer (one owner per record, then threshold),
...
def score(feat_dir: Path, booster: lgb.Booster, feats: list[str], log=print) -> pl.DataFrame:
    ...
    for i, p in enumerate(parts):
        ...
        df = df.select("s1", "m", "src").with_columns(pl.Series("p", booster.predict(X), dtype=pl.Float32))
...
def main():
    ...
    pred.write_parquet(args.out.parent / "artifacts" / "test_scores.parquet") if (args.out.parent / "artifacts").exists() else None
