...
Model: microsoft/mdeberta-v3-base (MIT, 278M params) with a single-logit head, BCE loss, 1 epoch, AMP, both T4s (DataParallel).
...
MODEL, MAXLEN, BS, LR, EPOCHS = "microsoft/mdeberta-v3-base", 128, 96, 2e-5, 1
t0 = time.time()
...
@torch.no_grad()
def predict(model, tok, df, bs=256):
    model.eval()
...
if __name__ == "__main__":
    ...
    model = AutoModelForSequenceClassification.from_pretrained(MODEL, num_labels=1, torch_dtype=torch.float32).float().cuda()
    ...
    for ep in range(EPOCHS):
        ...
        for enc in dl:
            ...
            if not torch.isfinite(loss):  # DeBERTa-v3 + fp16 can overflow on rare batches: skip them
        ...
        model.save_pretrained("/kaggle/working/ce_model")  # save first: a failure while scoring must not lose training
        tok.save_pretrained("/kaggle/working/ce_model")
        del opt, sch, scaler, net
        import gc
        gc.collect()
